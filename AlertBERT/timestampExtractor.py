...
def getSyslogTimestamp(line):
    ...
    if len(line) > 15:
        timestamp = datetime.datetime.strptime(
            "2022 " + line[:15], "%Y %b %d %H:%M:%S"
        )  # TODO be aware: syslog has no year, thus %Y is hardcoded!
        timestamp = timestamp.replace(tzinfo=pytz.utc)
    return timestamp
...
def getJsonTimestamp(line):
    ...
    if len(line) > 45:
        timestamp = datetime.datetime.strptime(
            line[14:45], "%Y-%m-%dT%H:%M:%S.%f%z"
        )  # parser.parse(line[14:45])
    return timestamp
...
def getFastTimestamp(line):
    ...
    if len(line) > 26:
        timestamp = datetime.datetime.strptime(
            line[:26], "%m/%d/%Y-%H:%M:%S.%f"
        )  # parser.parse(line[:26])
        timestamp = timestamp.replace(tzinfo=pytz.utc)
    return timestamp
...
def getAccessTimestamp(line):
    ...
    if len(parts) > 4:
        if parts[3][0] == "[":
            timestamp = datetime.datetime.strptime(
                parts[3][1:] + " " + parts[4][:-1], "%d/%b/%Y:%H:%M:%S %z"
            )
        else:
            timestamp = datetime.datetime.strptime(
                parts[4][1:] + " " + parts[5][:-1], "%d/%b/%Y:%H:%M:%S %z"
            )
    return timestamp
...
def getErrorTimestamp(line):
    ...
    if len(line) > 32:
        timestamp = datetime.datetime.strptime(
            line[5:32], "%b %d %H:%M:%S.%f %Y"
        )  # parser.parse(line[1:32])
        timestamp = timestamp.replace(tzinfo=pytz.utc)
    return timestamp
