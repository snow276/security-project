...
安全运营中心（SOC）每天接收数万条安全告警，但真实攻击仅占 0.01%。本项目以 AlertBERT 自监督聚类为基线，探索 LLM 在告警语义分析中的价值边界。核心发现：**聚类质量决定 LLM 分析上限，Few-shot 范式学习可将攻击阶段识别率从 49.3% 提升至 80.0%，单次分析成本约 $0.00026**。

---

## 目录

1. [故事一：AIT-ADS-A 数据集与 AlertBERT 基线](#故事一ait-ads-a-数据集与-alertbert-基线)
2. [故事二：Completeness 困境](#故事二completeness-困境)
3. [故事三：直接 LLM 分析的失败](#故事三直接-llm-分析的失败)
4. [故事四：Few-shot 攻击范式学习](#故事四few-shot-攻击范式学习)
5. [故事五：结论与关键发现](#故事五结论与关键发现)
6. [快速开始](#快速开始)
7. [项目结构](#项目结构)
8. [复现实验](#复现实验)
...
## 故事一：AIT-ADS-A 数据集与 AlertBERT 基线
...
上述结论仅统计了 attack-phase 簇的识别率。为了更严谨地评估，我们在 Scenario 7 上对 **全部 3,724 个簇** 进行了 Zero-shot 与 Few-shot 的完整精度评估（Precision / Recall / F1 / Accuracy）。
...
2. **Recall 提升明显**：Few-shot 帮助识别了更多真实攻击（60% vs 46.67%），范式学习确实有效
3. **FP 大幅减少**：Few-shot 将误报从 616 降到 259，减少 **58%**，说明范式中的 "看起来良性但实际是攻击" 的提示起到了关键作用
4. **Accuracy 大幅提升**：从 83% → 93%，因为大量 benign 簇被正确识别

**结论**：全簇评估精确量化了核心论点——在 0.01% 攻击率的 SOC 场景中，直接 LLM 分析所有簇的 Precision < 4%，不具备工程可用性。Few-shot 虽能显著降低假阳性（-58%），但 precision 仍然过低。这进一步验证了：**必须先改善上游聚类质量，再谈 LLM 语义分析**。
...
**攻击范式模板格式示例**（5 个阶段：侦察→初始访问/凭据攻击→执行/持久化→数据外泄→权限提升/横向移动）：
...
|------|------|--------|-------------|-----------|----------|------|------|---------|---------|
| S3 (santos) | A | 18,188 | 13 | 45.0% (9/20) | 85.0% (17/20) | +9 | -1 | $0.0076 | $0.0127 |
| S4 (shaw) | A | 24,364 | 12 | 55.6% (10/18) | 88.9% (16/18) | +7 | -1 | $0.0087 | $0.0137 |
| S6 (wheeler) | B | 6,681 | 12 | 50.0% (10/20) | 70.0% (14/20) | +5 | -1 | $0.0070 | $0.0129 |
| S7 (wilson) | A | 3,724 | 11 | 47.1% (8/17) | 76.5% (13/17) | +5 | 0 | $0.0063 | $0.0119 |
...
在 SOC 场景下，对 75 个 attack-phase 簇进行精确分诊的成本低于 4 美分，具备工程可行性。
...
3. **Few-shot 范式学习显著提升攻击识别召回率**：从 49.3% 提升至 80.0%，净增 23 个正确识别的攻击阶段簇
...
└── experiments/                 # 实验结果
    ...
    │   ├── summary.json         # 汇总：ZS=49.3%, FS=80.0%, cost=$0.0808
...
预期结果（汇总）：
- 总成本: $0.0808
- Zero-shot 攻击识别召回率: 49.3%
- Few-shot 攻击识别召回率: 80.0%
- 净提升: +23 个簇
- 升级: 26, 降级: 3
